        ...
        <div class="ff-page-header-actions">
            <div>
                <div class="ff-breadcrumb">
                    ...
                    <span class="text-gray-900">{{ __('Create') }}</span>
                </div>
                <h1 class="ff-page-title text-2xl">{{ __('Create User') }}</h1>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-lg mx-auto px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-6">
                ...
                <div class="ff-card">
                    <div class="p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <input id="email" name="email" type="email" class="ff-input" value="{{ old('email') }}" required />
                            <x-input-error class="mt-1" :messages="$errors->get('email')" />
                        </div>

                        <div>
                            <label for="password" class="ff-label">{{ __('Password') }} <span class="text-red-500">*</span></label>
                            <input id="password" name="password" type="password" class="ff-input" required />
                ...
                <div class="flex items-center gap-4">
                    <button type="submit" class="ff-btn-primary">{{ __('Create User') }}</button>
                    <a href="{{ route('admin.users.index') }}" class="ff-btn-secondary">{{ __('Cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
